...
export const createUser = async (req: Request, res: Response) => {
  ...
  try {
    // Check User Limit (Multi-tenant)
    const dbNameRes = await pool.query('SELECT current_database()')
    const dbName = dbNameRes.rows[0].current_database

    const projectRes = await mainPool.query('SELECT user_limit FROM erp_projects WHERE db_name = $1', [dbName])

    if (projectRes.rows.length > 0) {
      const userLimit = projectRes.rows[0].user_limit
      const countRes = await pool.query('SELECT COUNT(*) FROM users')
      const currentUserCount = parseInt(countRes.rows[0].count)

      if (currentUserCount >= userLimit) {
        return res.status(403).json({ error: 'forbidden', message: `User limit of ${userLimit} reached. Please contact your administrator to upgrade.` })
      }
    }
  ...
  try {
    ...
    if (req.user) {
      await pool.query(
        `INSERT INTO rbac_audit_log (user_id, action, details) 
         VALUES ($1, $2, $3)`,
        [req.user.userId, 'DELETE_USER', JSON.stringify({ userId: user.id, userEmail: user.email })]
      )
    }
