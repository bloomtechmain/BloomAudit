...
export const createEmployee = async (req: Request, res: Response) => {
  ...
  try {
    // 1. Check User Limit from Main System
    // Extract current DB name from connection string or env
    ...
    if (currentDbName) {
      ...
      if (limitResult.rows.length > 0) {
        const userLimit = limitResult.rows[0].user_limit
        ...
        if (currentCount >= userLimit) {
          return res.status(403).json({
            ...
            message: `User limit of ${userLimit} reached. Contact System Admin.`
          })
        }
      }
    }
