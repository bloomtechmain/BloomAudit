...
export const getAllProjects = async (req: Request, res: Response) => {
  try {
    const query = `
      ...
      GROUP BY p.project_id, p.project_name, p.description, p.status, p.created_at
...
export const createProject = async (req: Request, res: Response) => {
  ...
  try {
    const query = `
      ...
      VALUES ($1, $2, $3)
      RETURNING project_id, project_name, description AS project_description, status, created_at
    `
...
export const updateProject = async (req: Request, res: Response) => {
  ...
  try {
    const query = `
      UPDATE projects
      SET projects_name = $1, description = $2, status = $3
      WHERE project_id = $4
      RETURNING project_id, projects_name AS project_name, description AS project_description, status, created_at
    `
