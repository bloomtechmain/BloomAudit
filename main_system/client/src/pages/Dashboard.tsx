...
import ERPProjects from './ERPProjects'
import { LayoutDashboard, Users, ClipboardList, Store, FolderOpen, Banknote, Landmark, Receipt, Coins, Inbox, Plus, PlusCircle, CreditCard, Building2, ChevronLeft, ChevronRight, ChevronDown, BarChart3, Settings as SettingsIcon, DollarSign, TrendingDown, TrendingUp, Globe } from 'lucide-react'
...
export default function Dashboard({
  ...
  const [tab, setTab] = useState<'home' | 'employees' | 'projects' | 'accounting' | 'analytics' | 'assets' | 'settings' | 'erp_projects'>('home')
  ...
  const saveProjectItem = async () => {
    ...
    try {
      if (editingItem) {
        const r = await fetch(`${API_URL}/projects/${currentProjectForItems.project_id}/items/${encodeURIComponent(editingItem.requirements)}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${accessToken}`
        ...
        const r = await fetch(`${API_URL}/projects/${currentProjectForItems.project_id}/items`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${accessToken}`
  ...
  const updateEmployee = async () => {
    ...
    setSaving(true)
    try {
      const r = await fetch(`${API_URL}/employees/${editingEmployee.employee_id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${accessToken}`
  ...
  const deleteEmployee = async () => {
    ...
    setSaving(true)
    try {
      const r = await fetch(`${API_URL}/employees/${deletingEmployee.employee_id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${accessToken}` }
      })
      if (!r.ok) {
    ...
    <div style={{ height: '100vh', width: '100%', display: 'grid', gridTemplateRows: '56px 1fr', background: 'transparent', color: 'var(--text-main)', overflow: 'hidden' }}>
      <header className="glass-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', overflow: 'hidden', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <span>{user.name} ({user.roleName || 'No Role'})</span>
      ...
      <main style={{ height: 'calc(100vh - 56px)', padding: 0, display: 'flex', overflow: 'hidden' }}>
        <aside className="glass-sidebar" style={{ width: navOpen ? 240 : 64, transition: 'width 0.2s ease', height: '100%', display: 'flex', flexDirection: 'column', gap: 4, padding: 12 }}>
          ...
          {(user.roleName === 'Admin' || user.roleName === 'Super Admin') && (
            <button onClick={() => setTab('erp_projects')} title="Client Systems" style={{ display: 'flex', alignItems: 'center', justifyContent: navOpen ? 'flex-start' : 'center', gap: navOpen ? 12 : 0, padding: '10px 12px', borderRadius: 8, border: '1px solid var(--primary)', background: tab === 'erp_projects' ? 'var(--accent)' : 'transparent', color: '#fff', cursor: 'pointer' }}>
              <Globe size={20} />
              {navOpen && <span>Client Systems</span>}
            </button>
          )}
        ...
        <section style={{ flex: 1, overflowY: 'auto', background: 'transparent', borderRadius: 0, border: 'none', padding: 24 }}>
          ...
          {tab === 'erp_projects' && <ERPProjects accessToken={accessToken} />}
          {tab === 'settings' && <Settings accessToken={accessToken} />}
          {tab === 'assets' && (
            <div style={{ width: '100%', display: 'grid', gap: 16 }}>
